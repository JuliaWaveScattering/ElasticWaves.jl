...
function field_modes(wave::ElasticWave{2}, r::T, FT::FieldType,  boundarytype::BC) where {BC <: BoundaryCondition, T}

    pmodes = hcat([
        pressure_field_mode(wave.ω, r, wave.medium, wave.potentials[1].modes[i], FT,  boundarytype) * wave.potentials[1].coefficients[:,i]
    for i = eachindex(wave.potentials[1].modes)]...)

    smodes = hcat([
        shear_field_mode(wave.ω, r, wave.medium, wave.potentials[2].modes[i], FT, boundarytype) * wave.potentials[2].coefficients[:,i]
    for i = eachindex(wave.potentials[2].modes)]...)
...
function pressure_field_mode(ω::T, r::Union{T,Complex{T}}, medium::Elastic{2}, mode::Int, ::DisplacementType, boundarytype::BC) where {BC <: BoundaryCondition, T}
    ...
    if boundarytype.inner==true
        ...
        return hcat(bessel_modes(besselj), 0.0*bessel_modes(hankelh1))
    
    else

        return hcat(0.0*bessel_modes(besselj), bessel_modes(hankelh1))
...
function shear_field_mode(ω::T, r::Union{T,Complex{T}}, medium::Elastic{2}, mode::Int, ::DisplacementType, boundarytype::BC) where {BC <: BoundaryCondition, T}
...
function pressure_field_mode(ω::T, r::Union{T,Complex{T}}, medium::Elastic{2}, mode::Int, ::TractionType, boundarytype::BC) where {BC <: BoundaryCondition, T}
...
function shear_field_mode(ω::T, r::Union{T,Complex{T}}, medium::Elastic{2}, mode::Int, ::TractionType, boundarytype::BC) where {BC <: BoundaryCondition, T}
