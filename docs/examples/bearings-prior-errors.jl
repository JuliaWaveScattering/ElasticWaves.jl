...
## Do one example
    ...
    # fp_loading = (1.0 .- (1.0 + 0.0im .- cos.(loading_θs .- θo)) ./ (2ε))
    ...
    inds = findall(abs.(loading_profile.modes) .> 2)

    norm(loading_profile.coefficients)
    norm(loading_profile.coefficients[inds,:]) / norm(loading_profile.coefficients)
...
## Use loading prior 
    ...
    gr(size = (image_width * 1.6, image_width),linewidth = 2.0)
    

    s = [(θ < 3.1) ? -1.0 : 1.0 for θ in loading_θs]

    plot(loading_θs,  abs.(fp_loading), label = "Exact")
    plot!(loading_θs, mean(inv_loadings); ribbon = std(inv_loadings) .* 0.5, fillalpha=.4, linewidth = 2.0, linestyle = :dash, lab = "4 sensors", xlab = "θ", ylab = "loading profile")
    plot!(loading_θs, mean(inv_loadings_2); ribbon = std(inv_loadings_2) .* 0.5, fillalpha=.4, linewidth = 2.0, linestyle = :dash, lab = "7 sensors", xlab = "θ", ylab = "loading profile")
    # savefig("docs/images/bearing-prior-cos-loading.pdf")
    # savefig("docs/images/bearing-prior-loading.pdf")
    # savefig("docs/images/bearing-prior-smooth-loading.pdf")
